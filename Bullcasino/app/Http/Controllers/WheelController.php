...
class WheelController extends Controller
{
    ...
    public function wheel($color, Request $r) {
        ...
        // Auth check removed - auto-login handles this
        if(Auth::User()->ban == 1) return response()->json(['error' => 'true', 'message' => 'Your account is banned. Please contact support.']);
        if($game->status == 1 || $game->status == 2) return response()->json(['error' => 'true', 'message' => 'Betting is closed.<br> Wait for the next game']);
        if($color != 'black' && $color != 'yellow' && $color != 'red' && $color != 'green') return response()->json(['error' => 'true', 'message' => 'Error! Try again later']);
        if(Auth::User()->balance < $bet || !Auth::User()->balance) return response()->json(['error' => 'true', 'message' => 'Insufficient funds!']);
        if($bet < 1 || !$bet) return response()->json(['error' => 'true', 'message' => 'Minimum bet is 1 coin']);
        ...
        if($count_bets >= 2) return response()->json(['error' => 'true', 'message' => 'Maximum number of bets reached!<br> Wait for the next game']);
    ...
    public function adminwheel($color, Request $r) {
        ...
        if($game->status == 1) return response()->json(['error' => 'true', 'message' => 'Game has already started!']);
    ...
    public function infoWheel(Request $r) {
        ...
        if($count < 1) return response()->json(['error' => 'true', 'message' => 'Game not found']);
        if(!$game_id) return response()->json(['error' => 'true', 'message' => 'Missing parameter']);
        if(!Auth::check()) return response()->json(['error' => 'true', 'message' => 'XSRF token expired. Refresh the page']);
        $user_id = Auth::User()->id;
