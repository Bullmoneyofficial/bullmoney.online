...
class CrashController extends Controller
{
    ...
    public function index() {
        ...
        if($this->settings->crash_enabled != 1) return redirect('/')->with('error', 'This game mode is currently unavailable!');
    ...
    public function createBet(Request $r) {
        ...
        if($this->game->status > 0) return [
            'success' => false,
            'msg' => 'Bets for this round are closed!'
        ];

        if($bet < 1) return [
            'success' => false,
            'msg' => 'Minimum bet amount - 1'
        ];

        if($bet > 1000000) return [
            'success' => false,
            'msg' => 'Maximum bet amount - 1000000'
        ];
        ...
        try {
            ...
            if(!is_null($bet))
            {
                ...
                return [
                    'success' => false,
                    'msg' => 'You have already placed a bet this round!'
                ];
            }
    ...
    public function lastBet() {
        $bet = CrashBets::where('user_id', Auth::user()->id)->orderBy('id', 'desc')->first();
    ...
    public function Cashout() {
        ...
        if(is_null($bet)) return [
            'success' => false,
            'msg' => 'You didn\'t place a bet this round!'
        ];

        if($bet->status == 1) return [
            'success' => false,
            'msg' => 'You have already cashed out!'
        ];
        ...
        try {
            ...
            if($cashout == 0)
            {
                ...
                return [
                    'success' => false,
                    'msg' => 'You cannot cash out! The round hasn\'t started yet or has already ended...'
                ];
            }
            ...
            if($float <= 0)
            {
                ...
                return [
                    'success' => false,
                    'msg' => 'Something went wrong! Multiplier is zero!'
                ];
            }
            $user->update([
            ...
            return [
                'success' => true,
                'msg' => 'You have successfully cashed out!',
                'balance' => $user->balance,
