...
class JackpotController extends Controller
{
    ...
    public function index(Request $r)
    {
        ...
        if($settings->jackpot_enabled != 1) return redirect('/')->with('error', 'This game mode is currently unavailable!');
    ...
    {   
        $settings = Settings::orderBy('id', 'desc')->first();
        if($settings->jackpot_enabled != 1) return response()->json(['type' => 'error', 'msg' => 'This game mode is currently unavailable!']);
        ...
        if ($userbets >= 3) {
            return response()->json([
                'message' => 'You cannot place more than 3 bets per game!',
                'error' => 'true'
            ]);
        }
        ...
        if ($this->game->status == 2 || $this->game->status == 3) {
            return response()->json([
                'message' => 'Bets for this game are closed!',
                'error' => 'true'
            ]);
        }
        if (!$moneytick) {
            return response()->json([
                'message' => 'Enter a bet amount!',
                'error' => 'true'
            ]);
        }
        if ($moneytick > Auth::user()->balance) {
            return response()->json([
                'message' => 'Not enough coins to bet!',
                'error' => 'true'
            ]);
        }
        if ($moneytick < 1) {
