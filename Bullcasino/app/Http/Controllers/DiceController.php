...
class DiceController extends Controller
{
    public function dice(Request $r) {
        ...
        if($settings->dice_enabled != 1) return response()->json(['type' => 'error', 'msg' => 'This game mode is currently unavailable!']);
        ...
        if($bet > $balance) return response()->json(['type' => 'error', 'msg' => 'Insufficient funds']);
        if($bet < 1 || !is_numeric($bet)) return response()->json(['type' => 'error', 'msg' => 'Minimum bet amount is 1!']);
        if($per < 1 || $per > 90 || !is_numeric($per)) return response()->json(['type' => 'error', 'msg' => 'Enter a chance from 1 to 90']);
        if(is_null($type) || $type != 'min' && $type != 'max') return response()->json(['type' => 'error', 'msg' => 'Could not determine bet type']);
