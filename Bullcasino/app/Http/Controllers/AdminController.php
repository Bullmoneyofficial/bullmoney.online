...
class AdminController extends Controller
{
    ...
    public function manageUser($id, Request $r) {
        ...
        if($user->referred_by == null) $user_ref = 'No referrer';
    ...
    public function banUser($id, Request $r) {
        $user = User::where('id', $id)->first();
        if($user->ban == 1) return redirect()->back()->with('success', 'Account is already banned');
    ...
    public function unbanUser($id, Request $r) {
        $user = User::where('id', $id)->first();
        if($user->ban == 0) return redirect()->back()->with('success', 'Account is not banned!');
        User::where('id', $id)->update([
            'ban' => 0
        ]);

        return redirect()->back()->with('success', 'Account unbanned!');
    }
    ...
    public function userSave(Request $r) {
        ...
        return redirect('/admin/user/edit/'.$r->get('id'))->with('success', 'Information updated!');
    }
    ...
    public function withdraws() {
        ...
        foreach($list as $itm) {
            ...
            $time = $itm->created_at->diffInMinutes(Carbon::now()).' minutes ago';
            if($itm->created_at->diffInMinutes(Carbon::now()) > 60) {
                $time = $itm->created_at->diffInHours(Carbon::now()).' hours ago';
            } elseif($itm->created_at->diffInHours(Carbon::now()) >= 24){
                $time = $itm->created_at->diffInDays(Carbon::now()).' days ago';
            }
            $withdraws[] = [
                'id' => $itm->id,
                'user_id' => $user->id,
    ...
    public function fk_withdraw($id) {
        ...
        if($withdraw->status == 1 || $withdraw->status == 2) return redirect()->route('admin.withdraws')->with('error', 'Withdrawal cancelled or already processed');

        $data = array(
            'wallet_id'=>'',
            'purse'=>$withdraw->number,
            'amount'=>$withdraw->amount,
            'desc'=>'Withdraw BullMoney '.$withdraw->id,
            'currency'=>$system,
        ...
        if($json['status'] == 'error') {
            if($json['desc'] == 'Balance too low') {
                ...
                $desc = 'This user uses an anonymous wallet!';
                return redirect()->route('admin.withdraws')->with('error', $desc);
        ...
        if($json['status'] == 'info') {
            $withdraw->status = 1;
            $withdraw->save();
            return redirect()->route('admin.withdraws')->with('success', 'Withdrawal sent');
        }
    }
    ...
    public function cancel_withdraw($id) {
        ...
        return redirect()->route('admin.withdraws')->with('success', 'Withdrawal cancelled, returned to balance '.$withdraw->amount.'р');
    }
    ...
    public function promoNew(Request $r) {
        ...
        if(!$code) return redirect()->route('admin.promocodes')->with('error', 'The "Code" field cannot be empty!');
        if(!$amount) return redirect()->route('admin.promocodes')->with('error', 'The "Amount" field cannot be empty!');
        if(!$count_use) $count_use = 0;
        if($have) return redirect()->route('admin.promocodes')->with('error', 'This code already exists');
    ...
    public function promoDelete($id) {
        ...
        return redirect()->route('admin.promocodes')->with('success', 'Promo code successfully deleted!');
    }
    ...
    public function gameEnabled($game) {
        ...
        $currentValue = $settings->$game_enabled;
    ...
    public function stopWheel() {
        ...
        if($game->status == 2 || $game->status == 0) return response()->json(['type' => 'error', 'msg' => 'Game not started yet, global stop not required']);

        $this->redis = Redis::connection();
        $this->redis->publish("admin", json_encode(['type' => 'stopWheel']));

        return response()->json(['type' => 'success', 'msg' => 'Wheel restarted']);
    }
    ...
    public function restartWheel() {
        ...
        if($bets == 0) return response()->json(['type' => 'error', 'msg' => 'No bets, restart not required']);
        if($game->status == 1) return response()->json(['type' => 'error', 'msg' => 'Game already started, stop the spin first']);
