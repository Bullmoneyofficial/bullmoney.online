$(document).ready(function() {
    ...
    function withdraw(payway){
        ...
        if($('#amount').val() < withdraw_options[payway]['min']) {
            return noty('Minimum withdrawal amount - '+withdraw_options[payway]['min'], 'error');
        }
        if($('#amount').val() > withdraw_options[payway]['max']) {
            return noty('Maximum withdrawal amount - '+withdraw_options[payway]['max'], 'error');
        }
        $.post('/user/withdraw',{_token: $('meta[name="csrf-token"]').attr('content'), amount: amount, system: payway, number: wallet, final_amount: final_amount }).then(e=>{
            if(e.success){
                noty('Request successfully created', 'success');
    ...
    $('.pay__submit').click(function() {
        ...
        if($('#amount').val() < payment_options[payway]['min']) {
            return noty('Minimum deposit amount - '+payment_options[payway]['min'], 'error');
        }
        if($('#amount').val() > payment_options[payway]['max']) {
            return noty('Maximum deposit amount - '+payment_options[payway]['max'], 'error');
        }
        $.post('/wallet/pay/' + paysys, { _token: $('meta[name="csrf-token"]').attr('content'), size: $('#amount').val() }).then(e => {
            if (e.success) {
                noty('Redirecting...', 'success');
        ...
        if(e.success){
                ...
                e.history.forEach((e)=>{
                    ...
                    if (historyType == 0){
                        ...
                        if(status == 0){
                            st = '<td class="tb-center" style="color: #F4A900;display:flex;flex-direction:column;">Processing <a href="/wallet/cancel/withdraw/'+e.id+'" style="font-size: 14px; color: rgb(216,51,51); font-weight: 500;">Cancel</a></td>'
                        }
                        if(status == 1){
                            st = '<td class="tb-center" style="color: #2BD301;">Completed</td>'
                        }
                        if(status == 2){
                            st = '<td class="tb-center" style="color: #B5111E;">Cancelled</td>'
                        }
                        if(status == 3){
                            st = '<td class="tb-center" style="color: #7c7c8a;">Processing</td>'
                        }
                        ...
                        if(status == 0){
                            st = '<td class="tb-center" style="color: #F4A900;">Processing</td>'
                        }
