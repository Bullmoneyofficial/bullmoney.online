...
const TargetCursor = memo(({
  ...
  useEffect(() => {
    ...
    // Ticker for smooth corner animation
    const tickerFn = () => {
...
const GhostLoaderCursor = memo(() => {
  ...
  return (
    <div className="fixed inset-0 z-[60] pointer-events-none"> 
      <motion.div style={{ left: x3, top: y3 }} className="fixed w-32 h-32 rounded-full bg-blue-600/30 blur-[40px] -translate-x-1/2 -translate-y-1/2 mix-blend-screen" />
      <motion.div style={{ left: x2, top: y2 }} className="fixed w-12 h-12 rounded-full bg-blue-400/50 blur-[12px] -translate-x-1/2 -translate-y-1/2 mix-blend-screen" />
      <motion.div style={{ left: x, top: y }} className="fixed w-4 h-4 rounded-full bg-cyan-100 shadow-[0_0_40px_rgba(34,211,238,1)] -translate-x-1/2 -translate-y-1/2 z-10" />
    </div>
  );
});
      ...
      {loading && (
        ...
        >
          <div className="absolute inset-0 bg-[#010309]">
             <MatrixBackground />
             <GhostLoaderCursor />
             <motion.div animate={{ rotate: 360, scale: [1, 1.1, 1] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="absolute -top-[50%] -left-[50%] w-[200%] h-[200%] bg-[radial-gradient(circle_at_center,_rgba(23,37,84,0.15)_0%,_transparent_50%)] blur-[120px]" />
             <motion.div animate={{ rotate: -360, scale: [1, 1.2, 1] }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }} className="absolute -bottom-[50%] -right-[50%] w-[200%] h-[200%] bg-[radial-gradient(circle_at_center,_rgba(30,27,75,0.1)_0%,_transparent_50%)] blur-[120px]" />
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center justify-center w-full max-w-md px-4 pointer-events-none">
            ...
            <div className="relative w-48 h-48 mb-16 flex items-center justify-center perspective-[1000px]">
              <div className="absolute inset-0 bg-blue-900/10 rounded-full blur-[60px] animate-pulse" />
              <motion.div animate={{ rotateZ: 360, rotateY: [0, 15, 0, -15, 0], rotateX: [0, 10, 0, -10, 0] }} transition={{ rotateZ: { duration: 10, repeat: Infinity, ease: "linear" }, default: { duration: 6, repeat: Infinity, ease: "easeInOut" } }} className="absolute inset-0 rounded-full border-[3px] border-dashed border-blue-900/40 shadow-[0_0_20px_rgba(30,58,138,0.2)]" style={{ transformStyle: 'preserve-3d' }} />
              <motion.div animate={{ rotateZ: -360, rotateY: [0, -20, 0, 20, 0], rotateX: [0, -15, 0, 15, 0] }} transition={{ rotateZ: { duration: 12, repeat: Infinity, ease: "linear" }, default: { duration: 8, repeat: Infinity, ease: "easeInOut" } }} className="absolute inset-4 rounded-full border-[3px] border-dashed border-indigo-900/40 shadow-[0_0_20px_rgba(49,46,129,0.2)]" style={{ transformStyle: 'preserve-3d' }} />
              ...
              <svg className="absolute inset-0 w-full h-full -rotate-90 pointer-events-none z-10 overflow-visible" viewBox="0 0 160 160">
                <circle cx="80" cy="80" r="70" className="stroke-blue-950/30 stroke-[6] fill-none" />
                 <motion.circle cx="80" cy="80" r="70" className="stroke-blue-600/30 stroke-[8] fill-none blur-[12px]" strokeDasharray={circumference} initial={{ strokeDashoffset: circumference }} animate={{ strokeDashoffset: strokeDashoffset }} transition={{ ease: "linear", duration: 0.2 }} />
                <motion.circle cx="80" cy="80" r="70" className="stroke-blue-500 stroke-[4] fill-none drop-shadow-[0_0_10px_rgba(59,130,246,0.8)]" strokeDasharray={circumference} initial={{ strokeDashoffset: circumference }} animate={{ strokeDashoffset: strokeDashoffset }} transition={{ ease: "linear", duration: 0.2 }} strokeLinecap="round" />
              </svg>
            </div>
            {/* TEXT CONTENT */}
            <div className="flex flex-col items-center space-y-6 w-full">
              <div className="relative">
                <h1 className="text-5xl font-black text-blue-50 tracking-tighter relative z-10 text-shadow-[0_0_7px_#1e3a8a,0_0_15px_#1e3a8a,0_0_35px_#172554,0_0_60px_#172554]">BULLMONEY FREE</h1>
                <h1 className="text-5xl font-black text-blue-950 tracking-tighter absolute inset-0 blur-[8px] z-0 animate-pulse opacity-80">BULLMONEY FREE</h1>
              </div>
              <div className="w-full max-w-[280px] flex flex-col items-center justify-center relative p-4 bg-blue-950/10 rounded-xl border border-blue-800/30 backdrop-blur-md overflow-hidden">
                ...
                <motion.div key={progress} className="text-3xl font-mono font-bold text-blue-400 mb-2 tabular-nums tracking-widest drop-shadow-[0_0_10px_rgba(59,130,246,0.5)]">
                  {Math.floor(progress).toString().padStart(3, '0')}%
                </motion.div>
                <div className="h-6 flex items-center justify-center">
                    <EncryptedText text={loadingStates[currentStep]?.text || "LOADING"} key={currentStep} className="text-blue-200/80 font-bold text-xs tracking-[0.2em] uppercase" encryptedClassName="text-blue-800" revealedClassName="text-blue-200" interval={30} revealDelayMs={20} />
                </div>
              </div>
            </div>
          </div>
...
export default function RegisterPage({ onUnlock }: RegisterPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#010309] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className={cn("absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent to-transparent opacity-50 transition-colors duration-500", isVantage ? "via-purple-900" : "via-blue-900")} />
      <div className={cn("absolute bottom-0 right-0 w-[500px] h-[500px] rounded-full blur-[120px] pointer-events-none transition-colors duration-500", isVantage ? "bg-purple-900/10" : "bg-blue-900/10")} />

      <div className="w-full max-w-xl relative z-10">
        <div className="mb-8 text-center">
           <h1 className="text-2xl font-black text-white tracking-tight opacity-50">BULLMONEY <span className={cn("transition-colors duration-300", isVantage ? "text-purple-600" : "text-blue-600")}>COMMUNITY</span></h1>
        </div>
