        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h1 className="text-3xl font-black text-white tracking-tighter mb-4">
              BULLMONEY <span className="text-blue-500">AFFILIATE</span>
            </h1>
            <div className="h-8 overflow-hidden flex flex-col items-center">
              <AnimatePresence mode="wait">
                <motion.p
                  ...
                  className="text-white/60 font-mono text-sm tracking-widest uppercase text-center whitespace-nowrap"
                >
    ...
    >
      ...
      <div className="bg-[#0A1120] border border-blue-500/30 p-8 rounded-2xl shadow-[0_0_80px_rgba(59,130,246,0.2)] text-center max-w-lg w-full relative z-10">
        ...
        <p className="text-slate-400 mb-8 max-w-sm mx-auto">
          Welcome to the **BULLMONEY COMMUNITY**. Choose your path below.
        </p>
...
export default function RegisterPage({ onUnlock }: RegisterPageProps) {
  ...
  useEffect(() => {
    // Check URL params for ?ref=bmt_xxx
  ...
  const handleRegisterSubmit = async () => {
    ...
    try {
      ...
      // Check for referrer ID if a code exists
      ...
      // --- FIX: Store the CODE text, not a boolean 'used_code' ---
      const insertPayload = {
        email: formData.email,
        mt5_id: formData.mt5Number,
        password: formData.password,
        
        // Storing the specific code used (e.g., 'bmt_justin')
        referred_by_code: referralCode || null,
        
        // Storing the ID of the referrer
  ...
  if (loading) {
    return <MultiStepLoader loadingStates={loadingStates} loading={loading} duration={1200} />;
  }

  if (step === 5) {
    return <SuccessScreen onUnlock={onUnlock} />;
  }
