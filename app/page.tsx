...
// ----------------------------------------------------------------------
// 8. GESTURE NAVIGATION SYSTEM
// ----------------------------------------------------------------------
const GestureNavigation = ({ activePage, onNavigate }: { activePage: number; onNavigate: (page: number) => void }) => {
    ...
    <>
      ...
      <div className={currentStage === 'content' ? 'profit-reveal w-full h-[100dvh] relative' : 'opacity-0 pointer-events-none h-0 overflow-hidden'}>
        ...
        <main className="w-full h-full flex flex-col overflow-y-scroll overflow-x-hidden snap-y snap-mandatory scroll-smooth bg-black no-scrollbar text-white relative">
          ...
          {/* UNIFIED NAV - Same for Desktop & Mobile */}
          <div className="fixed right-8 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-6 items-center pointer-events-auto">
            <button onClick={() => scrollToPage(activePage - 2)} disabled={activePage === 1} className="text-blue-500 hover:text-white disabled:opacity-20 transition-colors">
              <ChevronUp size={24} />
            </button>
            <div className="flex flex-col gap-4 bg-black/40 backdrop-blur-xl p-3 rounded-2xl border border-white/5 shadow-2xl max-h-[50vh] overflow-y-auto no-scrollbar">
              {PAGE_CONFIG.map((page, index) => (
                <div key={page.id} className="relative group flex items-center justify-end gap-3">
                  ...
                  <ShineButton onClick={() => scrollToPage(index)} active={activePage === page.id} className="w-12 h-12 text-sm">
                    <span className="font-bold z-10">{index + 1}</span>
                  </ShineButton>
                </div>
              ))}
            </div>
            <button onClick={() => scrollToPage(activePage)} disabled={activePage === PAGE_CONFIG.length} className="text-blue-500 hover:text-white disabled:opacity-20 transition-colors">
              <ChevronDown size={24} />
            </button>
          </div>
          ...
          <div className="fixed top-24 left-4 z-50 md:bottom-8 md:top-auto md:left-8 pointer-events-auto">
            <ShineButton className="w-12 h-12 rounded-full" onClick={() => setModalData(PAGE_CONFIG[activePage - 1])}>
              <Info size={20} />
            </ShineButton>
          </div>
