...
import Faq from "./Faq"; // Assuming small component, keep direct import

// --- DYNAMIC IMPORTS (Reduce Initial Bundle Size) ---
const AdminLoginModal = dynamic(() => import("@/app/VIP/AdminLoginModal"), { ssr: false });
const AdminPanel = dynamic(() => import("@/app/VIP/AdminPanel"), { ssr: false });
...
// --- HOOK: SAFE MOBILE DETECTION (SSR Friendly) ---
const useIsMobile = () => {
...
// --- HOOK: ADMIN LOGIC SEPARATION ---
// Extracts the messy state management out of the visual component
const useProductAdmin = (
  ...
  // Reset form when active product changes
  useEffect(() => {
    if (activeProduct) {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    if (window.confirm("Are you sure? This cannot be undone.")) {
      ...
      try {
        ...
        setActiveProduct(null); // Close modal on delete
      } catch (error) {
  ...
  return { isEditing, setIsEditing, isSaving, editForm, setEditForm, handleSaveEdit, handleDelete };
};

// --- COMPONENT: SPARKLES (Optimized) ---
const SparklesCore = React.memo(({ isMobile }: { isMobile: boolean }) => {
  ...
  const options = useMemo(() => ({
    ...
    interactivity: {
      events: {
        // Completely disable mouse interaction on mobile to save listeners
  ...
  if (!init) return <div className="opacity-0" />;

  return (
    <div className="opacity-100 transition-opacity duration-1000 pointer-events-none absolute inset-0">
      <Particles id="tsparticles" className="h-full w-full" options={options as any} />
    </div>
  );
});
...
const ProductCard = React.memo(({
  ...
  return (
    <motion.div
      ...
      className="group/product h-[16rem] w-[12rem] xs:h-[20rem] xs:w-[16rem] md:h-[26rem] md:w-[22rem] relative flex-shrink-0 cursor-pointer backface-hidden"
    >
      <div className="block group-hover/product:shadow-[0_0_20px_rgba(14,165,233,0.3)] transition-shadow duration-500 rounded-[20px] h-full w-full bg-neutral-900 border border-neutral-800">
        <motion.div
          layoutId={uniqueLayoutId}
          transition={{ type: "spring", stiffness: 300, damping: 30 }}
          className="relative h-full w-full rounded-[20px] overflow-hidden bg-neutral-900 border border-transparent group-hover/product:border-sky-500/50 transition-colors"
        >
          <Image
            src={product.imageUrl}
            fill
            sizes="(max-width: 768px) 50vw, 20vw" // Precise sizes for performance
            loading="lazy"
            quality={70} // Slightly reduced quality for list view speed
            className="object-cover object-center absolute inset-0 opacity-80 group-hover/product:opacity-100 group-hover/product:scale-105 transition-all duration-700 will-change-transform"
            alt={product.name}
          />
            ...
            <div className="flex justify-between items-end">
              <div className="max-w-[85%]">
                <div className="flex items-center gap-2 mb-2">
                  <span className="px-2 py-0.5 rounded bg-sky-500/20 text-sky-400 text-[8px] md:text-[10px] font-bold uppercase tracking-wider border border-sky-500/20 backdrop-blur-md">
...
// --- MAIN HERO COMPONENT ---
const HeroParallax = ({ onScrollToProducts }: { onScrollToProducts: () => void }) => {
  ...
  const displayProducts = useMemo(() => {
    ...
    // Optimize: Don't duplicate too many times on mobile
  ...
  const rows = useMemo(() => ({
    first: displayProducts.slice(0, itemsPerRow),
    second: displayProducts.slice(itemsPerRow, itemsPerRow * 2),
    third: displayProducts.slice(itemsPerRow * 2, displayProducts.length)
  }), [displayProducts, itemsPerRow]);
  ...
  // Mobile Optimization: Zero out rotation on mobile to save GPU
  const rotateXVal = isMobile ? 0 : 15;
  const rotateZVal = isMobile ? 0 : 20;
  const translateXVal = isMobile ? 100 : 800;
  
  const translateX = useSpring(useTransform(scrollYProgress, [0, 1], [0, translateXVal]), springConfig);
  const translateXReverse = useSpring(useTransform(scrollYProgress, [0, 1], [0, -translateXVal]), springConfig);
  const rotateX = useSpring(useTransform(scrollYProgress, [0, 0.2], [rotateXVal, 0]), springConfig);
  const opacity = useSpring(useTransform(scrollYProgress, [0, 0.2], [0.2, 1]), springConfig);
  const rotateZ = useSpring(useTransform(scrollYProgress, [0, 0.2], [rotateZVal, 0]), springConfig);
  ...
  // Use custom hook for edit logic
  const { 
    isEditing, setIsEditing, isSaving, editForm, setEditForm, handleSaveEdit, handleDelete 
  } = useProductAdmin(activeProduct, updateProduct, deleteProduct, setActiveProduct);
  ...
  // Lock Body Scroll
  useEffect(() => {
    if (activeProduct) document.body.style.overflow = "hidden";
  ...
  return (
    <div className="bg-black relative selection:bg-sky-500/30 w-full overflow-hidden">
      
      {/* LAZY LOAD ADMIN MODAL */}
      <AdminLoginModal open={isAdminLoginOpen} onClose={() => setIsAdminLoginOpen(false)} />
      ...
      <AnimatePresence>
        {activeProduct && activeLayoutId && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[999] grid place-items-center bg-black/90 backdrop-blur-xl p-2 md:p-4 will-change-opacity"
            onClick={handleClose}
          >
            <motion.div
            ...
            >
              ...
              <div className="w-full md:w-1/2 p-4 sm:p-6 md:p-12 flex flex-col justify-start md:justify-center overflow-y-auto bg-neutral-900 text-neutral-100">
                {isEditing ? (
                  <div className="space-y-3 md:space-y-4 pb-10" onClick={(e) => e.stopPropagation()}>
                    {/* --- EDIT MODE --- */}
                    <div>
                  ...
                  <div className="pb-10 md:pb-0">
                    ...
                    <div className="flex justify-between items-center bg-black/50 p-3 rounded border border-sky-500/20 mb-8">
      ...
      >
        ...
        <div className="max-w-7xl relative mx-auto py-12 md:py-32 px-4 w-full z-20 mb-20 md:mb-32">
          ...
          <SparklesCore isMobile={isMobile} />
        ...
        {/* PARALLAX GRID */}
        {/* will-change used via hook on child props, style block kept simple */}
        <motion.div
          style={{ rotateX, rotateZ, translateY, opacity, willChange }}
      ...
      {isAdmin && (
        <div className="max-w-7xl mx-auto px-4 mt-10 relative z-20 mb-20">
          <AdminPanel editing={adminEditing} clearEditing={() => setAdminEditing(null)} />
        </div>
      )}
