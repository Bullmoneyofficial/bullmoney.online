...
// --- HOOK: DETECT MOBILE (Optimized with debounce) ---
const useIsMobile = () => {
...
const SparklesCore = React.memo((props: {
  id?: string;
  className?: string;
  background?: string;
  minSize?: number;
  ...
  const options = useMemo(() => ({
    ...
    interactivity: {
      events: {
        // Disable hover on mobile to save listeners
    ...
    particles: {
      ...
      move: {
        enable: true,
        speed: isMobile ? speed * 0.5 : speed, // Slower on mobile
        direction: "none",
        random: false,
        straight: false,
      ...
      number: {
        ...
        value: isMobile ? Math.min(particleDensity, 20) : particleDensity, // HARD LIMIT particles on mobile
      },
      opacity: {
    ...
    detectRetina: true,
  }), [background, particleColor, speed, particleDensity, minSize, maxSize, isMobile]);
...
const ProductCard = React.memo(({
  ...
  // Use Framer's willChange to hint browser about upcoming transforms
  ...
  return (
    ...
    >
      <div className="block group-hover/product:shadow-[0_0_20px_rgba(14,165,233,0.3)] transition-shadow duration-500 rounded-[20px] h-full w-full">
        <motion.div 
            layoutId={uniqueLayoutId}
            transition={{ type: "spring", stiffness: 300, damping: 30 }} // Explicit transition to prevent layout thrashing
        ...
        >
            <Image
                ...
                fill
                sizes="(max-width: 768px) 50vw, 25vw" // Important for mobile data/render speed
                loading="lazy"
                quality={75}
                className="object-cover object-center absolute h-full w-full inset-0 opacity-80 group-hover/product:opacity-100 group-hover/product:scale-105 transition-all duration-700 will-change-transform"
                alt={product.name}
            />
            {/* Gradient Overlay - using simpler CSS */}
            ...
            <div className="absolute bottom-0 left-0 w-full p-3 md:p-5 translate-y-2 group-hover/product:translate-y-0 transition-transform duration-500">
                <div className="flex justify-between items-end">
                    ...
                    <div className="hidden md:flex h-8 w-8 rounded-full bg-sky-600 text-white items-center justify-center opacity-0 group-hover/product:opacity-100 transition-all scale-75 group-hover/product:scale-100">
                        <TrendingUp size={16} />
                    </div>
                </div>
            </div>
...
const HeroParallax = ({ onScrollToProducts }: HeroProps) => {
  ...
  // OPTIMIZATION: Reduce translation distance heavily on mobile
  const maxTranslate = isMobile ? 150 : 800;
  const maxRotate = isMobile ? 5 : 15;
  const maxTranslateY = isMobile ? [-100, 0] : [-700, 200]; // Less vertical movement on mobile

  // Looser spring for mobile = less CPU calculation per frame
  ...
  return (
                    ...
                    <div className="w-full md:w-1/2 p-4 sm:p-6 md:p-12 flex flex-col justify-start md:justify-center overflow-y-auto bg-neutral-900 text-neutral-100">
                        {isEditing ? (
                          <div className="space-y-3 md:space-y-4 pb-10" onClick={(e) => e.stopPropagation()}>
                             {/* EDIT FORM (No Animations for inputs to be snappy) */}
                             <div>
    ...
    <div
        ref={ref}
        // Use transform: preserve-3d to enable hardware acceleration
    ...
    >
        ...
        <div className="max-w-7xl relative mx-auto py-12 md:py-32 px-4 w-full z-20 mb-20 md:mb-32">
             <div className="absolute inset-0 w-full h-full z-0 pointer-events-none opacity-60">
                {/* Memoized Sparkles to prevent re-renders on scroll */}
                <SparklesCore
                    id="parallax-sparkles"
                    background="transparent"
                    minSize={0.6}
                    maxSize={1.4}
                    particleDensity={50} // Handled inside component for mobile check
                    isMobile={isMobile}
        ...
        {/* PARALLAX GRID */}
        {/* ADDED will-change-transform to hint browser */}
        <motion.div
