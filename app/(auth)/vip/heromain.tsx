...
import LogoImage from "@/public/images/logos/bullmoney-logo.png"; 
      ...
      {!decoded ? (
        ...
        >
            ...
            <motion.div
                ...
                className="absolute top-0 bottom-0 w-[20px] bg-linear-to-r from-transparent via-white/50 to-transparent -skew-x-12 z-10"
            />
        </motion.div>
      )}
    </div>
    ...
    >
      ...
      <div 
        className={cn(
            ...
            isTop ? "-translate-y-1 border-b border-r border-white/10" : "translate-y-1 border-t border-l border-white/10"
        )}
      />
      ...
      <div className="relative p-[1.5px] overflow-hidden rounded-full shadow-lg">
          ...
          />
          <div className="relative z-10 px-3 py-1 bg-neutral-900 rounded-full flex items-center justify-center border border-white/10 min-h-7.5 min-w-20">
              {text ? (
...
const ShimmerBorder = ({ 
  ...
  return (
    <div className={cn("relative p-px overflow-hidden group/shimmer", rounded, className)}>
      <div className="absolute inset-0 bg-[linear-gradient(110deg,transparent,45%,#ffffff,55%,transparent)] bg-size-[250%_100%] animate-shimmer opacity-100" />
...
const VideoCard = React.memo(({
  ...
  return (
    ...
    >
      <div className="relative block h-full w-full md:group-hover/product:shadow-[0_0_40px_rgba(255, 255, 255,0.2)] transition-all duration-500 rounded-[20px] md:rounded-3xl safari-fix-layer">
        <AnimatePresence>
            {showTip && (
        ...
        <motion.div 
            ...
            className="relative h-full w-full rounded-[20px] md:rounded-3xl overflow-hidden bg-neutral-900 border border-white/30 md:group-hover/product:border-white/50 transition-colors safari-mask-fix"
        >
            {videoId ? (
...
const HeroParallax = ({ embedded = false }: { embedded?: boolean }) => {
  ...
  return (
    ...
    <AnimatePresence>
        {activeProduct && activeLayoutId && (
            <motion.div
                ...
                onClick={handleClose}
            >
                <div onClick={(e) => e.stopPropagation()} className="w-full max-w-7xl h-dvh md:h-[85vh] md:max-h-200">
    ...
    <div
        ref={ref}
        className="min-h-screen h-auto md:h-[240vh] pt-10 pb-20 md:pb-0 overflow-visible md:overflow-hidden bg-black antialiased relative flex flex-col self-auto perspective-[1000px] transform-3d"
    >
        {/* HEADER SECTION */}
