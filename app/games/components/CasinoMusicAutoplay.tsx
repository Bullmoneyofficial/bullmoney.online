...
export function CasinoMusicAutoplay({ enabled = true }: CasinoMusicAutoplayProps) {
  ...
  }, [enabled, playlist, playlistIndex]);

  useEffect(() => {
    if (!enabled || !autoplayBlocked) return;
  ...
  }, [enabled, autoplayBlocked]);

  if (!enabled) return null;

  return <audio ref={audioRef} aria-hidden className="hidden" playsInline />;
}
