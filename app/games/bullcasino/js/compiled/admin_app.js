...
$(document).ready(function () {
		...
		$.post('/admin/enabled/dice', { _token: $('meta[name="csrf-token"]').attr('content') }).then(e => {
			if (e.success) {
				if(e.active == 'active') {
					...
					noty('Mode disabled', 'success');
				} else {
					$('.game__toggle_dice').addClass('active');
					noty('Mode enabled', 'success');
				}
			}
			if (e.error) {
	...
	$(".stopWheel").click(function() {
		$('.stopWheel').attr('disabled', true).html('Processing..');
		$.ajax({
			url: '/admin/stopWheel',
			type: 'POST',
			success: function (data) {
	...
	$('.withdraw__multi_show').click(function() {
		...
		$.ajax({
            ...
            success : function(data) {
				...
				if (html.length == 0) {
                    html = '<span class="aw_green">No multi-accounts detected</span>';
                }
