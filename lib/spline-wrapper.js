...
const getQualityProfile = (tier) => {
  ...
  const profiles = {
    high: {
      ...
      maxTextureSize: 4096,
      frameRateLimit: nativeHz, // Up to 120Hz on ProMotion/high-refresh displays
      loadDelay: 0, // Instant load
      rootMargin: '800px', // Preload earlier
    },
    medium: {
      ...
      loadDelay: 0, // Instant load
      rootMargin: '600px', // Preload earlier
    },
    low: {
      dpr: 1,
      antialias: false,
...
export default function SplineWrapper({ 
  ...
  return (
    ...
    >
      ...
      <canvas
        ref={canvasRef}
        className={`block outline-none transition-opacity duration-700 ease-out ${isLoaded && !loadError ? 'opacity-100' : 'opacity-0'}`}
        style={{ 
          width: '100%',
          height: '100%',
          maxWidth: '100%',
