...
if (typeof window !== 'undefined') {
  ...
  if (!heroScenePreloaded) {
    ...
    const isDesktop = window.innerWidth >= 1024 && !/mobi|android|iphone|ipad|ipod/i.test(navigator.userAgent);
    
    // Only hard-preload the hero scene on true desktop + good network.
    // This avoids competing with critical JS/CSS on constrained connections.
    if (isDesktop && !saveData && !isSlowNet) {
      const existing = document.querySelector('link[rel="preload"][href="/scene1.splinecode"]');
      if (!existing) {
    ...
    // Method 2: Cache warming (Cache API). Avoid storing large ArrayBuffers in memory.
    if (isDesktop && !saveData && !isSlowNet) {
...
export default function SplineWrapper({ 
  ...
  neverDisable = false, // NEW: CRITICAL - bypasses battery saver and emergency shutdown for welcome screen
  targetFPS,
  maxDpr,
  minDpr,
  ...
  const memoryIntervalsRef = useRef([]);
  ...
  // Ensure any internal intervals are cleaned up (prevents long-session leaks)
  useEffect(() => {
    return () => {
      try {
        const intervals = memoryIntervalsRef.current || [];
        for (let i = 0; i < intervals.length; i++) {
          clearInterval(intervals[i]);
        }
        memoryIntervalsRef.current = [];
      } catch {}
    };
  }, []);
  ...
  // SKIPPED when neverDisable=true (for welcome screen hero)
  // ========================================
  useEffect(() => {
    if (typeof window === 'undefined') return;
    
    // CRITICAL: Skip battery saver for neverDisable scenes (welcome screen hero)
    if (neverDisable) {
      console.log('[SplineWrapper] 🔒 neverDisable=true: battery saver DISABLED for this scene');
      return;
    }
  ...
  // ========================================
  // THERMAL OPTIMIZATION - Respond to thermal-state-change events
  // NOTE: ALL thermal levels keep Spline rendering - just at reduced quality
  // User requirement: "all must render still"
  // ========================================
  useEffect(() => {
    if (typeof window === 'undefined') return;

    const handleThermalChange = (e) => {
      const { state: thermalState, config } = e.detail || {};
      if (!thermalState || !config) return;

      console.log(`[SplineWrapper] 🌡️ Thermal state: ${thermalState.thermalLevel}`);

      // Critical thermal state - KEEP rendering but at absolute minimum quality
      // DO NOT shutdown - user wants content to always show
      if (thermalState.thermalLevel === 'critical') {
        console.log('[SplineWrapper] 🚨 Critical thermal - minimum quality (still rendering)');
        setQualityProfile(prev => ({
          ...prev,
          frameRateLimit: 15, // Very low FPS
          dpr: Math.min(prev.dpr, 0.5), // Minimum DPR
        }));
        if (limiterRef.current) {
          limiterRef.current.targetFps = 15;
        }
        // Ensure we're still running, not paused
      ...
      // Hot thermal state - reduce quality significantly
      if (thermalState.thermalLevel === 'hot') {
        console.log('[SplineWrapper] 🔥 Hot thermal - reducing quality');
        setQualityProfile(prev => ({
          ...prev,
          frameRateLimit: Math.min(prev.frameRateLimit, 30),
          dpr: Math.min(prev.dpr, 0.75),
        }));
        if (limiterRef.current) {
          limiterRef.current.targetFps = 30;
        }
        return;
      }

      // Warm thermal state - mild reduction
      if (thermalState.thermalLevel === 'warm') {
        setQualityProfile(prev => ({
          ...prev,
          frameRateLimit: Math.min(prev.frameRateLimit, 45),
          dpr: Math.min(prev.dpr, 1.0),
        }));
        if (limiterRef.current) {
          limiterRef.current.targetFps = 45;
        }
        return;
      }

      // Cool - restore normal operation if we were throttled
      if (emergencyShutdown && thermalState.thermalLevel === 'cool') {
        console.log('[SplineWrapper] ❄️ Thermal cool - restoring');
        setEmergencyShutdown(false);
        if (appRef.current?.play) {
          try { appRef.current.play(); } catch {}
        }
    ...
    window.addEventListener('thermal-state-change', handleThermalChange);
    return () => window.removeEventListener('thermal-state-change', handleThermalChange);
  }, [shouldLoadInstantly, emergencyShutdown]);

  // Emergency shutdown monitor - DISABLED FOR HERO and neverDisable scenes to ensure they always show
  useEffect(() => {
    // HERO OVERRIDE: Never shutdown hero scenes to ensure they always show
    // BATTERY SAVER: Skip if in battery saving mode
    // NEVER DISABLE: Skip if neverDisable is true (welcome screen hero MUST always show)
    if (!isLoaded || emergencyShutdown || shouldLoadInstantly || isBatterySaving || neverDisable) return;
    ...
    const monitorPerformance = () => {
      ...
      // When hidden, don't do any emergency monitoring work.
      ...
      try {
        ...
        // Detect sustained main-thread stalls (GC thrash / CPU overload)
        ...
        if (performanceHistory.current.length >= 30) {
          ...
          // Emergency shutdown if we see both: extremely low responsiveness + sustained stalls.
          // This is a last-resort safety valve; it should be rare on desktop.
          if ((estimatedFPS < 10 || stallCount > 8) && !emergencyShutdown) {
            console.warn(`[SplineWrapper] Emergency shutdown triggered: ${estimatedFPS.toFixed(1)}fps (stalls=${stallCount})`);
    ...
    // Low-frequency monitor (1Hz) — avoids constant CPU work (helps laptop thermals).
    emergencyCheckInterval.current = setInterval(monitorPerformance, 1000);
    ...
    const init = async () => {
      ...
      try {
        ...
        if (isMobileDevice) {
          ...
          // Store interval for cleanup (per component instance)
          memoryIntervalsRef.current.push(memoryCheckInterval);
        }
  ...
  useEffect(() => {
    ...
    const pauseRendering = () => {
      ...
      // Pause/stop the Spline app (different runtime versions expose different APIs)
      if (appRef.current?.stop) {
        try { appRef.current.stop(); } catch {}
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {emergencyShutdown && (
          <div className="absolute inset-0 bg-linear-to-br from-white/20 via-white/10 to-black/40 flex items-center justify-center">
      ...
      {loadError && !emergencyShutdown && !isBatterySaving && (
        <div className="absolute inset-0 bg-linear-to-br from-white/20 via-white/10 to-black/40" />
      )}
      
      {/* 3D CANVAS - ALWAYS INTERACTIVE on mobile and desktop */}
      {/* Ultra-fast opacity transition for hero mode */}
      {/* BATTERY SAVER: Don't render canvas when in battery saving mode UNLESS neverDisable=true */}
      {/* neverDisable ensures welcome screen Spline ALWAYS shows on ALL devices */}
      {(neverDisable || (!emergencyShutdown && !isBatterySaving)) && (
        <canvas
          ref={canvasRef}
