...
// ALWAYS preload Spline runtime for hero - no browser restrictions
// Hero must load in 100ms on ALL browsers and devices
...
const getDeviceTier = (forceLoad = false) => {
  ...
  // If forceLoad is true (e.g., hero scene), skip browser restrictions
  // Hero MUST load on all devices
  if (!forceLoad) {
    // Check browser capabilities for non-hero scenes
    const browserInfo = detectBrowser();
    
    // In-app browsers get 'low' tier for non-hero scenes (reduced quality)
    if (browserInfo.isInAppBrowser) {
      return 'low';
    }
    
    // Very low memory devices get 'low' tier
...
export default function SplineWrapper({ 
  ...
  useEffect(() => {
    // For hero scenes, always use at least 'low' tier (never disabled)
    const tier = getDeviceTier(shouldLoadInstantly);
    setDeviceTier(tier);
    
    // Only disable for non-hero scenes
    if (tier === 'disabled' && !shouldLoadInstantly) {
      setIsDisabled(true);
      setLoadError(true); // Trigger fallback UI
      console.log('🔒 3D disabled for this browser - showing fallback');
      return;
    }
    
    // Hero scenes ALWAYS load - use 'low' tier at minimum
    const effectiveTier = shouldLoadInstantly && tier === 'disabled' ? 'low' : tier;
    
    setQualityProfile(getQualityProfile(effectiveTier, shouldLoadInstantly));
    console.log(`🎮 Spline Quality: ${effectiveTier.toUpperCase()} tier ${shouldLoadInstantly ? '(HERO MODE - INSTANT)' : ''}`);
  ...
  useEffect(() => {
    // Skip loading entirely for disabled browsers
    if (isDisabled || qualityProfile.disabled) {
