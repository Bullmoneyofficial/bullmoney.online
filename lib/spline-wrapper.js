...
if (typeof window !== 'undefined') {
  // Start runtime import immediately on module load
  ...
  // Preload hero scene immediately (don't wait for component mount)
...
const getDeviceTier = (forceLoad = false) => {
  ...
  // In-app browsers get 'low' tier for non-hero scenes
  if (isInAppBrowser) {
    return 'low';
  }
...
const getQualityProfile = (tier, isHero = false, overrides = {}) => {
  ...
  const reduceAnimations = typeof document !== 'undefined' && document.documentElement.classList.contains('reduce-animations');
  const splineQuality =
    typeof document !== 'undefined' && document.documentElement.classList.contains('spline-quality-low')
      ? 'low'
      : typeof document !== 'undefined' && document.documentElement.classList.contains('spline-quality-medium')
        ? 'medium'
        : 'high';
  ...
  if (isHero) {
    ...
    const dpr = clamp(Math.min(rawDpr, gpuCap, maxDpr), minDpr, gpuCap);
    return {
      ...
      frameRateLimit: Math.min(nativeHz, (typeof overrides.targetFPS === 'number' ? overrides.targetFPS : (isMobile ? 60 : 90))),
      loadDelay: 0,
      ...
      try {
        ...
        const getStableRect = () => {
          ...
          if (!rect || rect.width === 0 || rect.height === 0) {
            ...
            const fallbackWidth = parentRect?.width || Math.min(window.innerWidth, 960);
            const fallbackHeight = parentRect?.height || Math.min(window.innerHeight, 640);
            return {
              width: Math.max(240, Math.min(fallbackWidth, 1280)),
              height: Math.max(200, Math.min(fallbackHeight, 720)),
            };
          }
