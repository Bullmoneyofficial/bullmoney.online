...
export default function SplineWrapper({ 
  ...
  useEffect(() => {
    ...
    if (shouldLoadInstantly) {
      ...
      // Smart tier selection for hero (never disable, just optimize)
      let heroTier = 'high';
      if (width < 400 || memory < 2) {
        heroTier = 'low'; // Ultra-low quality for tiny screens/memory
      } else if (width < 768 || memory < 4) {
        heroTier = 'medium'; // Balanced for mobile
      } else if (memory >= 8 && cores >= 8) {
        heroTier = 'ultra'; // Premium for high-end devices
      }
      
      setDeviceTier(heroTier);
      setQualityProfile(getQualityProfile(heroTier, true, { targetFPS, maxDpr, minDpr }));
      console.log(`🚀 HERO Spline: ${heroTier.toUpperCase()} tier (50MS TARGET)`);
