...
export default function SplineWrapper({ 
  ...
  useEffect(() => {
    ...
    const pollId = window.setInterval(handleQualityChange, 2000);
  ...
  useEffect(() => {
    ...
    // Desktop heartbeat to catch late layout shifts; lightweight (1s).
    if (typeof window !== 'undefined' && window.innerWidth >= 1024) {
      heartbeatId = window.setInterval(() => {
        handleResize([{ contentRect: containerRef.current?.getBoundingClientRect() || { width: lastWidth, height: lastHeight } }]);
      }, 1000);
    }
