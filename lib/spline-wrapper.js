...
// Check if browser can handle 3D before preloading
let browserCanHandle3D = true;
if (typeof window !== 'undefined') {
  const browserInfo = detectBrowser();
  browserCanHandle3D = browserInfo.canHandle3D;
  
  // ONLY preload runtime if device can handle 3D
  if (browserCanHandle3D) {
    runtimePromise = import('@splinetool/runtime').then(mod => {
      ...
      browserCanHandle3D = false;
      return null;
    });
  } else {
    console.log('📱 Spline disabled for this browser:', browserInfo.browserName);
  }
}
...
const getDeviceTier = () => {
  ...
  // In-app browsers always get 'disabled' tier (no 3D)
...
export default function SplineWrapper({ 
  ...
  useEffect(() => {
    ...
    // Check if 3D is completely disabled for this browser
    if (tier === 'disabled') {
      setIsDisabled(true);
