...
const getDeviceTier = (forceLoad = false) => {
  ...
  // UPDATED 2026: Enhanced device detection for better Spline performance
  const browserInfo = typeof window.detectBrowser === 'function' ? window.detectBrowser() : null;
  
  // For hero scenes, respect browser detection but provide fallback
  if (forceLoad) {
    if (browserInfo?.shouldDisableSpline) {
      return 'minimal'; // Will trigger fallback instead of blocking completely
    }
...
const getQualityProfile = (tier, isHero = false, overrides = {}) => {
  ...
  if (isHero) {
    ...
    return {
      ...
      antialias: true, // UPDATED 2026: Enable antialiasing for all devices
      powerPreference: 'high-performance', // UPDATED 2026: High performance for all devices
  ...
  const profiles = {
    ultra: {
      ...
      antialias: true, // UPDATED 2026: Enable antialiasing for mobile
...
export default function SplineWrapper({ 
  ...
  useEffect(() => {
    ...
    if (!isLoaded || emergencyShutdown || shouldLoadInstantly) return;
  ...
  useEffect(() => {
    ...
    // Check for WebGL support - critical for preventing crashes
    let hasWebGL = true;
    ...
    // CRITICAL: Skip Spline on devices that will crash
    if (!hasWebGL) {
      console.warn('[SplineWrapper] WebGL not supported - showing fallback');
      setIsDisabled(true);
      setLoadError(true);
      return;
    }
    
    // For very low-end mobile in-app browsers, show fallback to prevent crashes
    if (isInAppBrowser && isLowEndMobile && !shouldLoadInstantly) {
      console.warn('[SplineWrapper] Low-end mobile in-app browser detected - showing fallback');
      setIsDisabled(true);
      setLoadError(true);
      return;
    }
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            console.error('[SplineWrapper] Fallback init also failed:', fallbackError.message);
            setLoadError(true);
            setIsDisabled(true);
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        maxWidth: isInAppBrowser ? '100vw' : '100%',
        maxHeight: isInAppBrowser ? '100dvh' : '100%',
        width: '100%',
        height: '100%',
        position: 'relative',
        touchAction: shouldLoadInstantly ? 'pan-y' : 'pan-y', // HERO: Always allow scroll
    ...
    >
      ...
      <div 
        className={`absolute inset-0 bg-cover bg-center ease-out ${isLoaded && !emergencyShutdown ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        style={{ 
      ...
      >
        ...
        {!placeholder && !isLoaded && !loadError && !shouldLoadInstantly && !emergencyShutdown && (
        ...
        {emergencyShutdown && (
          <div className="absolute inset-0 bg-gradient-to-br from-blue-900/20 via-purple-900/10 to-black/40 flex items-center justify-center">
      ...
      {loadError && !emergencyShutdown && (
        <div className="absolute inset-0 bg-gradient-to-br from-blue-900/20 via-purple-900/10 to-black/40" />
      )}
      
      {/* 3D CANVAS - PERFECT SCROLL HANDLING ON ALL DEVICES */}
      {/* Ultra-fast opacity transition for hero mode with scroll protection */}
      {!emergencyShutdown && (
        <canvas
          ref={canvasRef}
          ...
          style={{ 
            ...
            maxHeight: '100%',
            objectFit: 'contain',
            touchAction: shouldLoadInstantly ? 'pan-y' : (isInteracting ? 'none' : 'pan-y'), // HERO: Always allow scroll
            pointerEvents: shouldLoadInstantly ? 'auto' : 'auto', // HERO: Always interactive
            transition: shouldLoadInstantly ? 'opacity 50ms' : 'opacity 700ms', // HERO: 50ms target
            WebkitTouchCallout: 'none',
            WebkitUserSelect: 'none',
            userSelect: 'none',
          }}
          onPointerDown={shouldLoadInstantly ? undefined : handleInteractionStart} // HERO: Skip interaction mode
          onTouchStart={shouldLoadInstantly ? undefined : handleInteractionStart} // HERO: Skip interaction mode
        />
      )}
      
      {/* Dev indicator (only in development) */}
      {process.env.NODE_ENV === 'development' && isLoaded && !emergencyShutdown && (
