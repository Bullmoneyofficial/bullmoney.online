...
export const UltimateHubCommunityPostsTab = memo(() => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f5f7] overflow-hidden">
      ...
      <div className="shrink-0 bg-black/95 sticky top-0 z-10 border-b border-black/10">
        {/* Title + Search + Actions */}
        <div className="px-1.5 sm:px-2 py-1 flex items-center gap-1 sm:gap-2">
          {/* Title */}
          <span className="text-[8px] sm:text-[9px] font-bold text-black shrink-0">Feed</span>
          <span className="text-[7px] sm:text-[8px] text-black/40 shrink-0">({filteredAnalyses.length})</span>
            ...
            <div className="flex-1 min-w-0">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search..."
                  className="w-full pl-5 pr-2 py-0.5 bg-white/50 border border-zinc-700/50 rounded text-[8px] text-black placeholder:text-neutral-600 focus:outline-none focus:border-white/50"
                />
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 shrink-0 ml-auto">
            {isMobile && (
              <button
                ...
                className="p-1 rounded text-black/40 hover:text-zinc-300"
                aria-label="Search"
              >
            ...
            <button
              ...
              className={`p-1 rounded transition-colors ${showFilters ? 'text-black' : 'text-black/40'}`}
              aria-label="Filters"
            >
        ...
        {showFilters && (
          <div className="px-1.5 sm:px-2 pb-1 overflow-x-auto scrollbar-hide border-t border-black/[0.06]">
              ...
              {tabOptions.map(({ id, label, icon: Icon }) => (
                <button
                  ...
                  className={`flex items-center gap-0.5 px-1 py-0.5 rounded text-[7px] sm:text-[8px] font-medium whitespace-nowrap transition-colors ${
                    activeTab === id ? 'bg-white text-black' : 'text-black/40 hover:text-zinc-300'
                  }`}
                >
              ...
              {contentTypeOptions.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-0.5 px-1 py-0.5 rounded text-[7px] sm:text-[8px] font-medium whitespace-nowrap transition-colors ${
                      isActive ? 'bg-white/70 text-black' : 'text-black/40 hover:text-zinc-300'
                    }`}
                  >
                    <Icon className="w-2 h-2 sm:w-2.5 sm:h-2.5" />
                    <span className="hidden sm:inline">{label}</span>
      ...
      {selectedAnalysis && (
        <div className={`shrink-0 border-t border-black/10 bg-black/95 flex flex-col ${shouldSkipHeavyEffects ? '' : 'backdrop-blur-sm'}`}>
          {/* Header */}
          <div className="flex items-center justify-between px-3 py-2.5 sm:py-2 border-b border-black/[0.06] shrink-0">
            <div className="text-sm sm:text-sm text-black truncate flex-1 min-w-0 font-medium">
              <span className="hidden sm:inline">Comments · </span>
              <span className="truncate">{selectedAnalysis.title}</span>
            </div>
            <button
              onClick={() => setSelectedAnalysis(null)}
              className="ml-2 p-2.5 rounded-xl bg-[#f5f5f7]/50 text-black/50 hover:text-black hover:bg-zinc-700 shrink-0 transition-all active:scale-95"
