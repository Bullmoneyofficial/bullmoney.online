...
export default function AffiliatePage({ searchParams }: { searchParams?: { src?: string } }) {
  ...
  const [currentStage, setCurrentStage] = useState<"affiliate" | "v2" | "hold" | "content">("affiliate");
  ...
  const activeTheme = useMemo(() => {
    ...
    return ALL_THEMES.find((t: Theme) => t.id === activeThemeId) || ALL_THEMES[0];
  }, [activeThemeId]);
      ...
      <div className="relative w-full min-h-screen" onClick={safePlay}>
        ...
        <div className={currentStage === 'content' ? 'profit-reveal' : 'opacity-0 pointer-events-none h-0 overflow-hidden'}>
            ...
            <Navbar 
                ...
                onThemeChange={(themeId: string) => handleThemeChange(themeId, { name: 'MECHANICAL', volume: 50 }, isMuted)}
            />
