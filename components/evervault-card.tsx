...
export const EvervaultCard = ({ text, className }: { text?: string; className?: string }) => {
  ...
  return (
    <div className={cn("p-0.5 bg-transparent aspect-square w-full h-full relative", className)}>
      <div onMouseMove={onMouseMove} className="group/card rounded-3xl w-full h-full relative overflow-hidden bg-transparent flex items-center justify-center">
        ...
        <div className="relative z-10 flex items-center justify-center">
          <div className="relative h-44 w-44 rounded-full flex items-center justify-center text-4xl font-bold">
            ...
            <span className="z-20 dark:text-white text-black">{text}</span>
          </div>
        </div>
      </div>
    </div>
...
export function CardPattern({ mouseX, mouseY, randomString }: any) {
  ...
  return (
    <div className="pointer-events-none">
      ...
      <motion.div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-white to-white opacity-0 group-hover/card:opacity-100 backdrop-blur-xl transition duration-500" style={style} />
