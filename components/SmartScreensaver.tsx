...
export const SmartScreensaverProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Check idle status every 5 seconds (same in dev)
    const checkInterval = 5000;
    idleCheckIntervalRef.current = setInterval(() => {
      const now = Date.now();
      const idleTime = now - lastActivityRef.current;
      // 1 hour in dev, 5 minutes in production
      const IDLE_THRESHOLD = process.env.NODE_ENV === 'development' ? 3600000 : 300000;
      ...
      if (idleTime >= IDLE_THRESHOLD && !isScreensaverActive) {
        ...
        console.log(`[BULLMONEY] 🖥️ Showing screensaver - user idle for ${process.env.NODE_ENV === 'development' ? '1 hour (dev)' : '5+ minutes'}`);
