...
import './LaserFlow.css'; // Ensure this file exists (see Part 3 below)
...
export const LaserFlow: React.FC<Props> = ({
  className,
  style,
  wispDensity = 1.2, // Slightly increased density
  dpr,
  mouseSmoothTime = 0.1, // Slight smoothing on mouse interaction
  mouseTiltStrength = 0.02,
  horizontalBeamOffset = 0.0, // Centered horizontally
  verticalBeamOffset = -0.1, // Moved slightly down so the bright spot isn't at the very top
  flowSpeed = 0.2, // Slower, more majestic flow for a hero banner
  verticalSizing = 1.0, // Reduced vertical stretching
  horizontalSizing = 1.8, // Increased horizontal stretching for a wider beam
  fogIntensity = 0.6, // Increased fog for atmosphere
  fogScale = 0.25,
  wispSpeed = 10.0, // Slower wisps
  wispIntensity = 4.0,
  flowStrength = 0.3,
  decay = 1.1,
  falloffStart = 1.2,
  fogFallSpeed = 0.4,
  color = '#0055FF' // Bullmoney Blue (Electric Blue)
}) => {
  ...
  useEffect(() => {
    ...
    // Important: absolute positioning to sit behind content
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRGB(color || '#0055FF');
