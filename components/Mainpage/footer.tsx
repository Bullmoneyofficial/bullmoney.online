    ...
    <AnimatePresence mode="wait">
      {isOpen && (
        <motion.div 
          ...
          className="fixed inset-0 z-[999999] flex items-center justify-center p-3 sm:p-4 md:p-6"
        >
          {/* Backdrop */}
          <div 
            onClick={onClose}
          ...
          <motion.div
            ...
            className={cn(
              "relative w-full overflow-hidden rounded-2xl sm:rounded-3xl shadow-2xl shadow-blue-500/30",
              maxWidth
            )}
          ...
          >
            ...
            {/* Inner Content Area */}
            <div className="relative z-10 m-[2px] flex max-h-[85vh] sm:max-h-[80vh] flex-col rounded-2xl sm:rounded-3xl bg-black overflow-hidden">
              ...
              <div className="relative flex items-center justify-between border-b border-blue-500/30 px-4 sm:px-6 py-3 sm:py-4 bg-neutral-950 shrink-0">
                ...
                <div className="relative text-base sm:text-lg font-semibold tracking-wide text-white">
                  {title}
                </div>
                ...
                <button
                  onClick={onClose}
                  className="relative z-[9999999] p-2 sm:p-2.5 rounded-full bg-neutral-900 border border-blue-500/40 hover:border-blue-400/70 text-neutral-400 hover:text-white hover:bg-blue-500/20 transition-all duration-300 min-w-[40px] min-h-[40px] sm:min-w-[44px] sm:min-h-[44px] flex items-center justify-center group"
                >
              ...
              {/* Scrollable Body */}
              <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 footer-scrollbar relative bg-neutral-950">
...
const DisclaimerSection = ({ number, title, text }: { number: string; title: string; text: string }) => (
  <div className="group relative rounded-xl overflow-hidden transition-all duration-300">
    ...
    <div className="relative p-3 sm:p-4">
      <h3 className="flex items-center gap-2 sm:gap-3 font-semibold text-white mb-2 text-sm sm:text-base">
        <span className="flex h-5 w-5 sm:h-6 sm:w-6 items-center justify-center rounded-lg bg-blue-500/20 text-[9px] sm:text-[10px] font-bold text-blue-400 font-mono border border-blue-500/30">
          {number}
        </span>
        {title}
      </h3>
      <p className="text-neutral-400 text-xs sm:text-sm pl-7 sm:pl-9 leading-relaxed">
        {text}
      </p>
    </div>
  </div>
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            {apps.map((app, idx) => (
              ...
              >
                <h3 className="text-center text-xs sm:text-sm font-bold uppercase tracking-widest text-blue-400/80">
                  {app.title}
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
                  {app.links.map((link, i) => (
                    <Link
                      ...
                      href={link.href}
                      target="_blank"
                      className="group relative flex items-center justify-between rounded-lg sm:rounded-xl overflow-hidden p-3 sm:p-4 text-xs sm:text-sm text-neutral-300 transition-all duration-300"
                    >
                      {/* Solid dark background - NO blur */}
                      <div className="absolute inset-0 bg-neutral-900 border border-blue-500/20 group-hover:border-blue-500/50 rounded-lg sm:rounded-xl transition-all duration-300" />
                      ...
                      <ExternalLink className="relative z-10 h-3 w-3 opacity-0 transition-all group-hover:opacity-100 text-blue-400" />
                    </Link>
                  ))}
      ...
      <EnhancedModal 
        ...
        title={
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="text-sm sm:text-base">Legal & Financial Disclaimer</span>
          </div>
        }
      >
        <div className="flex flex-col h-full">
          ...
          <div className="space-y-4 sm:space-y-6 text-sm leading-relaxed text-neutral-400">
            ...
            {/* ALERT BANNER */}
            <div className="relative overflow-hidden rounded-xl border border-red-500/30 bg-red-950/50 p-4 sm:p-5">
              {/* Subtle glow - no blur */}
              ...
              <div className="relative flex gap-3 sm:gap-4">
                ...
                <div>
                  <h2 className="text-xs sm:text-sm font-bold text-white tracking-wide uppercase mb-1">Critical Legal Notice</h2>
                  <p className="text-[10px] sm:text-xs text-red-200/70">
            ...
            <p className="italic text-neutral-500 text-[10px] sm:text-xs text-center px-2 sm:px-4">
          ...
          {/* FIXED ACTION FOOTER */}
          <div className="mt-4 sm:mt-6 pt-4 border-t border-blue-500/20 flex justify-end shrink-0">
            <motion.button 
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-2 text-black">
                I Agree & Understand
                <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-600 opacity-0 group-hover:opacity-100 transition-all -ml-2 group-hover:ml-0" />
              </span>
            </motion.button>
          </div>
